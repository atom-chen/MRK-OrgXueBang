...
- (void)stopProximityMonitering {
    ...
    [[AVAudioSession sharedInstance] setCategory:AVAudioSessionCategoryPlayback error:nil];
    [[AVAudioSession sharedInstance] setActive:YES error:nil];
    [[UIDevice currentDevice] setProximityMonitoringEnabled:NO];//有从非主线程调用的异常，暂时先屏蔽
    NSLog(@"关闭距离监听");
}
...
//    AVURLAsset *urlAsset = [AVURLAsset URLAssetWithURL:filename options:opts]; // 初始化视频媒体文件
//    NSUInteger second = 0;
//    second = urlAsset.duration.value / urlAsset.duration.timescale; // 获取视频总时长,单位秒
//    return second;
    AVURLAsset *audioAsset=[AVURLAsset assetWithURL:filename];
