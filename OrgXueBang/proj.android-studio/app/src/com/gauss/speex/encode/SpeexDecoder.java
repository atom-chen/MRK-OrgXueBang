...
public class SpeexDecoder {
	...
	public void decode() throws Exception {
		...
		try {
			...
			while (true) {
                ...
                //while (this.isPaused()) {
                if (this.isPaused()) {//修改暂停即是结束
                    track.stop();
                    //Thread.sleep(100);
                    throw new EOFException("custom end");
                }
			...
			SpeexUtil.stopPlayRecordCallback();
		}
	...
	private boolean readSpeexHeader(final byte[] packet, final int offset, final int bytes, boolean init) throws Exception {
		...
		System.out.println("Decode mode=" + mode + " sampleRate==" + sampleRate + " channels=" + channels + " nframes=" + nframes + " framesize="
				+ frameSize);
