...
void CCBuffer::writeLong(long  data)
{
#if MEMORYTYPE_REVERSE
	DO_ASSIGN(char p_data[sizeof(long )], {0});
	memcpy(p_data, (char*)(QZ(data)), sizeof(long ));
	std::reverse(QZ(p_data[0]), QZ(p_data[sizeof(long )]));
	writeData(p_data, sizeof(long ));
#else
	DO_ASSIGN(char* p_data, (char*)(QZ(data)));
	writeData(p_data, sizeof(long ));
#endif
...
void CCBuffer::writeULong(unsigned long  data)
{
	writeLong((long)(data));
}
...
long  CCBuffer::readLong()
{
	BEGIN_IF(isReadable(sizeof( long)))
#if MEMORYTYPE_REVERSE
	DO_ASSIGN(char p_temp[sizeof( long)], {0});
	memcpy(p_temp, CA(_p_buffer, _u_read_pos), sizeof( long));
	std::reverse(QZ(p_temp[0]), QZ(p_temp[sizeof( long)]));
	DO_ASSIGN( long ret, QV(( long*)p_temp));
#else
	DO_ASSIGN( long ret, QV(( long*)(CA(_p_buffer, _u_read_pos))));
#endif
	AA(_u_read_pos, sizeof( long));
	DO_RETURN_R(ret);
	BEGIN_ELSE
	DO_RETURN_Z;
	END_IF
...
unsigned  long CCBuffer::readULong()
{
	DO_RETURN_R((unsigned long )(readLong()));
}
